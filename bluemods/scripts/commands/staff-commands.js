...
Command.register(
    ...
    (data, args) => {
        ...
        if (!["add", "list", "remove"].includes(action)) {
            player.sendMessage(
                `§7[§b#§7] §cInvalid action! §aUse this method§7: §3!ban §aadd §7[§aduration§7] ${main.player} <§areason§7> §7/ §3!ban §cremove ${main.player} §7/ §3!ban §alist`
            );
            system.run(() => player.runCommand("playsound random.break @s"));
            return;
        }
...
Command.register(
    ...
    (data, args) => {
        ...
        if (!["add", "remove", "list"].includes(action)) {
            player.sendMessage(
                "§7[§b#§7] §cInvalid action! §aUse this Method§7: §3!banitem §aadd §7<§aitem§7> / §3!banitem §cremove §7<§aitem§7> / §3!banitem §alist."
            );
...
Command.register(
    ...
    async (data, args) => {
        ...
        if (action === "list") {
            const frozenPlayers = world
                .getPlayers()
                .filter((p) => p.hasTag("freezed"))
                .map((p) => p.name)
                .join("§7,§r ");
            player.sendMessage(`§7[§b#§7] §aFrozen: §e${frozenPlayers || "No frozen player found"}`);
            return;
        }

        if (!["add", "remove"].includes(action)) {
            player.sendMessage(
                `§7[§b#§7] §cInvalid action! §aUse this Method§7: §3!freeze §aadd ${main.player} §7/ §3!freeze §cremove ${main.player} §7/ §3!freeze §alist`
            );
            system.run(() => player.runCommand("playsound random.break @s"));
            return;
        }
        ...
        try {
            if (action === "add") {
                ...
                if (!targetPlayer.hasTag("freezed")) {
                    ...
                    player.runCommand(`playsound note.bell @s`);
                    player.runCommand(`inputpermission set "${targetPlayer.name}" camera disabled`);
                    player.runCommand(`inputpermission set "${targetPlayer.name}" movement disabled`);
                    ...
                    system.run(() => player.runCommand("playsound random.break @s"));
                    player.sendMessage(`§7[§b#§7] §c${targetPlayer.name} this player is already frozen`);
                }
            } else if (action === "remove") {
                if (targetPlayer.hasTag("freezed")) {
                    await system.waitTicks(1);
                    ...
                    system.run(() => player.runCommand("playsound random.break @s"));
                    player.sendMessage(`§7[§b#§7] §c${targetPlayer.name} this player is not frozen.`);
                }
            }
        } catch (error) {
...
Command.register(
    ...
    (data, args) => {
        ...
        system.run(() => {
            try {
                player.runCommand(`give @s ${item} ${parsedAmount} ${dataValue}`);
...
Command.register(
    ...
    async (data, args) => {
        ...
        if (action === "list") {
            const notifyPlayers = world
                .getPlayers()
                .filter((p) => p.hasTag("notify"))
                .map((p) => p.name)
                .join(", ");
        ...
        if (!["add", "remove"].includes(action)) {
            player.sendMessage(
                `§7[§b#§7] §cInvalid action! §aUse this Method§7: §3!notify §aadd ${main.player} §7/ §3!notify §cremove ${main.player} §7/ §3!notify §alist`
            );
            system.run(() => player.runCommand("playsound random.break @s"));
            return;
        }
...
Command.register(
    ...
    async (data, args) => {
        ...
        if (action === "list") {
            const adminPlayers = world
                .getPlayers()
                .filter((p) => p.hasTag("admin"))
                .map((p) => p.name)
                .join(", ");
        ...
        if (!["add", "remove"].includes(action)) {
            player.sendMessage(
                `§7[§b#§7] §cInvalid action! §aUse this Method§7: §3!op §aadd ${main.player} §7/ §3!op §cremove ${main.player} §7/ §3!op §alist`
            );
            system.run(() => player.runCommand("playsound random.break @s"));
            return;
        }
...
Command.register(
    ...
    (data, args) => {
        ...
        if (args.length < 3 || args.length > 4) {
            player.sendMessage(
                `§7[§b#§7] §cInvalid action! §aUse this Method§7: §3!rank ${main.addremove} §7<§arank§7> §7[§gcolor(optional)§7] ${main.player}`
            );
            return system.run(() => player.runCommand(`playsound random.break @s`));
        }
        ...
        if (args.length === 4) {
            ...
            if (main.colors[colorName]) {
                ...
                player.sendMessage(
                    `§7[§b#§7] §cInvalid color! §aAvailable colors§7: §0black §7| §1dark_blue §7| §2dark_green §7| §3dark_aqua §7| §4dark_red §7| §5dark_purple §7| §6gold §7| §7gray §7| §8dark_gray §7| §9blue §7| §agreen §7| §baqua §7| §cred §7| §dlight_purple §7| §eyellow §7| §fwhite`
                ); ///////////////
                return system.run(() => player.runCommand(`playsound random.break @s`));
            }
        }
...
Command.register(
    ...
    (data, args) => {
        ...
        if (!trollTypes.includes(trollType)) {
            player.sendMessage(
                "§7[§b#§7] §aInvalid troll type. You have to choose one of these §7(§ecreeper§7/§eendermen§7/§eghast§7/§ezombie§7/§eskeleton§7)"
            );
            return system.run(() => player.runCommand("playsound random.break @s"));
        }
...
Command.register(
    ...
    async (data, args) => {
        ...
        if (action === "list") {
            const trustedPlayers = world
                .getPlayers()
                .filter((p) => p.hasTag("trusted"))
                .map((p) => p.name)
                .join(", ");
        ...
        if (!["add", "remove"].includes(action)) {
            player.sendMessage(
                `§7[§b#§7] §cInvalid action! §aUse this Method§7: §3!trusted §aadd ${main.player} §7/ §3!trusted §cremove ${main.player} §7/ §3!trusted §alist`
            );
            system.run(() => player.runCommand("playsound random.break @s"));
            return;
        }
