...
world.beforeEvents.chatSend.subscribe((data) => {
    ...
    const cmdData = Commands.registeredCommands.find(c => c.name === cmd || c.aliases?.includes(cmd));
     if (!cmdData) return player.runCommandAsync(`tellraw @s {"rawtext":[{"text":"§7[§c-§7] §cUnknown command§7: §g${message.replace(prefix, '')}§c. Please check that the command exists and that you have permission to use it."}]}`)
     
    if (cmdData.permission && !cmdData.permission(player)) return player.runCommandAsync(`tellraw @s {"rawtext":[{"text":"§7[§c-§7] §cUnknown command: §g${message.replace(prefix, '')}§c. Please check that the command exists and that you have permission to use it."}]}`)
